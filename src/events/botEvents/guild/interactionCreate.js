...
module.exports.run = async (client, interaction) => {
    if (interaction.type === InteractionType.ApplicationCommand) {
        ...
        const row = new ActionRowBuilder().addComponents(
            new ButtonBuilder().setLabel("Vote Me").setURL(voteUrl).setStyle(ButtonStyle.Link),
        );
        ...
        if (userBan && userBan.isBanned === true && interaction.user.id !== client.owner) {
            return interaction.reply({
                content: `<:xx:1202361957928144968> | You are banned from using ${client.user}, click the button support to appeal.`,
                components: [row],
                ephemeral: true,
            });
        }
        ...
        if (botMissingPermissions.length > 0) {
            return interaction.reply({
                content: `<:xx:1202361957928144968> | I don't have one of these permissions \`ViewChannel\`, \`SendMessages\`, \`EmbedLinks\`.\nPlease double check them in your server role & channel settings.`,
                components: [row],
                ephemeral: true,
            });
        }
        ...
        if (!interaction.guild.members.cache.get(client.user.id).permissions.has(command.permissions.bot || [])) {
            warning.setDescription(`<:xx:1202361957928144968> | I don't have permission \`${command.permissions.bot.join(", ")}\` to execute this command.`);
        ...
        if (!interaction.member.permissions.has(command.permissions.user || [])) {
            warning.setDescription(
                `<:xx:1202361957928144968> | You don't have permission \`${command.permissions.user.join(", ")}\` to execute this command.`,
            );
        ...
        if (command.settings.player && !player) {
            warning.setDescription(`<:xx:1202361957928144968> | There isn't player exists for this server.`);
        ...
        if (command.settings.current && !player.currentTrack) {
            warning.setDescription(`<:xx:1202361957928144968> | There isn't any current playing right now.`);
        ...
        if (command.settings.inVc) {
            if (!channel) {
                warning.setDescription(`<:xx:1202361957928144968> | You must be in a voice channel to use this command.`);
            ...
            if (
                ...
                warning.setDescription(
                    `<:xx:1202361957928144968> | I don't have permission \`${command.permissions.channel.join(
                        ", ",
        ...
        if (command.settings.sameVc) {
            if (!channel || interaction.member.voice.channelId !== interaction.guild.members.me.voice.channelId) {
                warning.setDescription(`<:xx:1202361957928144968> | You must be on the same voice channel as mine to use this command.`);
        ...
        if (command.settings.premium) {
            if (user && !user.isPremium) {
                warning.setDescription(`<:xx:1202361957928144968> | You're not premium user!`);
        ...
        if (command.settings.owner && interaction.user.id !== client.owner) {
            warning.setDescription(`<:xx:1202361957928144968> | Only my owner can use this command!\n\n© Sound Wave`);
        ...
        try {
            ...
            warning.setDescription(`<:xx:1202361957928144968> | Something went wrong.`);
