...
module.exports.run = async (client, message) => {
    ...
    if (message.content.match(mention)) {
        const embed = new EmbedBuilder().setColor(client.color).setDescription(`👋🏻 Hey Users\n<a:right_arrow:1201284536760291328> The Prefix for Sound Wave in this server is: \`/\` ( Slash Commands )`);
    ...
    const row = new ActionRowBuilder().addComponents(new ButtonBuilder().setLabel("Vote Me").setURL(voteUrl).setStyle(ButtonStyle.Link));
    ...
    if (command.owner && message.author.id !== client.owner) {
        return message.reply({ content: `<:xx:1202361957928144968> | Only my owner can use this command!\n\n© Sound Wave 2024` });
    }
    ...
    try {
        ...
        return message.reply({ content: `<:xx:1202361957928144968> | Something went wrong.`, components: [row] });
    }
};
