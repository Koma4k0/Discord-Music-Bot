...
module.exports = {
    ...
    run: async (client, interaction) => {
        ...
        if (!value) {
            const embed = new EmbedBuilder().setColor(client.color).setDescription(`\<a:Volume_Up:1201284996879626280>\ | Current player volume: \`${player.volume}%\``);

            return interaction.editReply({ embeds: [embed], ephemeral: true });
        } else {
            await player.setVolume(value);

            const embed = new EmbedBuilder().setColor(client.color).setDescription(`\<a:Volume_Up:1201284996879626280>\ | Volume has been set to: \`${value}%\``);
