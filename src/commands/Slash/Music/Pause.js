...
module.exports = {
    ...
    run: async (client, interaction) => {
        ...
        if (!player.isPaused) {
            ...
            const embed = new EmbedBuilder().setColor(client.color).setDescription(`\<a:play_pause:1201283574062661653>\ | Song has been: \`Paused\``);

            return interaction.editReply({ embeds: [embed] });
        } else {
            const embed = new EmbedBuilder().setColor(client.color).setDescription(`\<:icon_cross:1201253258749030440>\ | Song is already: \`Paused\``);
