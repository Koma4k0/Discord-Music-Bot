...
module.exports = {
    ...
    run: async (client, interaction) => {
        ...
        if (from === to || isNaN(from) || from < 1 || from > player.queue.length || isNaN(to) || to < 1 || to > player.queue.length)
            return interaction.editReply(`\<:icon_cross:1201253258749030440>\ | That song does not exist in the queue.`);

        const moved = player.queue[from - 1];
        await moveArrayElement(player.queue, from - 1, to - 1);

        const embed = new EmbedBuilder().setColor(client.color).setDescription(`\<:Check:1201254751833178213>\ | Moved \`${moved.info.title}\` to \`${to}\`.`);
