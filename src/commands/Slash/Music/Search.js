    ...
    run: async (client, interaction) => {
        ...
        if (player && interaction.member.voice.channelId !== interaction.guild.members.me.voice.channelId) {
            const warning = new EmbedBuilder()
                ...
                .setDescription(`\<:icon_cross:1201253258749030440>\ | You must be on the same voice channel as mine to use this command.`)
                .setTimestamp();
        ...
        if (/^https?:\/\/(?:www\.)?[-a-zA-Z0-9@:%._\+~#=]{1,256}\.[a-zA-Z0-9()]{1,6}\b(?:[-a-zA-Z0-9()@:%_\+.~#?&\/=]*)$/.test(query)) {
            const embed = new EmbedBuilder()
                .setDescription(`\<:icon_cross:1201253258749030440>\ | Please use \`/play\` command for url search query.`)
                .setColor(client.color);
            ...
            try {
                selectMenuCollector.on("collect", async (menu) => {
                    if (menu.user.id !== interaction.member.id) {
                        const unused = new EmbedBuilder().setColor(client.color).setDescription(`\<:icon_cross:1201253258749030440>\ | This menu is not for you!`);
                    ...
                    const tplay = new EmbedBuilder()
                        .setColor(client.color)
                        .setDescription(
                            `\<:queue:1201253104482533429>\ | **[${trackTitle ? trackTitle : "Unknown"}](${track.info.uri})** • \`${
                ...
                selectMenuCollector.on("end", async (collected) => {
                    if (!collected.size) {
                        const timed = new EmbedBuilder().setColor(client.color).setDescription(`\<:icon_cross:1201253258749030440>\ | Search was time out.`);
